package sguiSwing.menus

import clearmap.backend.IMasterService
import sgui.UIPoint
import sgui.menus.ICommandExecutor
import sgui.menus.IContextMenus
import sgui.menus.MenuItem
import sguiSwing.hybrid.Hybrid
import sguiSwing.hybrid.MDebug
import javax.swing.*

class SwContextMenus (private  val _commandExecuter : ICommandExecutor) : IContextMenus<IMasterService>
{
    private val cmenu = JPopupMenu()

    override fun LaunchContextMenu(point: UIPoint, scheme: List<MenuItem<IMasterService>>, obj: Any?) {
        cmenu.removeAll()

        constructMenu(cmenu, scheme.toList(), obj)
        cmenu.show( (point as? sguiSwing.SUIPoint)?.component, point.x, point.y)

        SwingUtilities.invokeLater { cmenu.requestFocus() } // Meh
    }

    fun constructMenu(root: JComponent, menuScheme: List<MenuItem<IMasterService>>, extra: Any? = null) {
        ...
        menuScheme.forEachIndexed { index, item ->
            ...
            if( depth > activeDepth) {
                MDebug.handleWarning(MDebug.WarningType.INITIALIZATION, "Bad Menu Scheme.")
                depth = activeDepth
            }
            activeDepth = depth+1
